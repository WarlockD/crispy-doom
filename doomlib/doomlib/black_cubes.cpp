...
void PObject::DrawFunc() { DrawPObject(this); }	 /* draws object */
void PObject::RecalcFunc() { XformAndProjectPObject(this); }	 /* prepares object for drawing */
void PObject::MoveFunc() { RotateAndMovePObject(this); }		  /* moves object */
...
void InitializeCubes()
{
	...
	for (i = 0; i<NUM_CUBES; i++) {
		assert((WorkingCube = new PObject) != nullptr);
		...
		for (k = 0; k<2; k++) {
			WorkingCube->EraseRect.Left =
				WorkingCube->EraseRect.Top = 0x7FFF;
			WorkingCube->EraseRect.Right = 0;
			WorkingCube->EraseRect.Bottom = 0;
		}
		...
		assert((WorkingCube->XformedVertexList = new Point3[NUM_CUBE_VERTS]) != nullptr);
		assert((WorkingCube->ProjectedVertexList = new Point3[NUM_CUBE_VERTS]) != nullptr);
		assert((WorkingCube->ScreenVertexList = new Point[NUM_CUBE_VERTS]) != nullptr);
		assert((WorkingCube->FaceList = new Face[NUM_CUBE_FACES]) != nullptr);
