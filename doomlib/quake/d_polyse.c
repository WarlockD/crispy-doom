...
split:
// split this edge
	nnew[0] = (lp1[0] + lp2[0]) >> 1;
	nnew[1] = (lp1[1] + lp2[1]) >> 1;
	nnew[2] = (lp1[2] + lp2[2]) >> 1;
	nnew[3] = (lp1[3] + lp2[3]) >> 1;
	nnew[5] = (lp1[5] + lp2[5]) >> 1;
...
// draw the point if splitting a leading edge
	...
	z = nnew[5]>>16;
	zbuf = zspantable[nnew[1]] + nnew[0];
	if (z >= *zbuf)
	{
		int		pix;
		
		*zbuf = z;
		pix = d_pcolormap[skintable[nnew[3]>>16][nnew[2]>>16]];
		d_viewbuffer[d_scantable[nnew[1]] + nnew[0]] = pix;
	}

nodraw:
// recursively continue
	D_PolysetRecursiveTriangle (lp3, lp1, nnew);
	D_PolysetRecursiveTriangle (lp3, nnew, lp2);
}
