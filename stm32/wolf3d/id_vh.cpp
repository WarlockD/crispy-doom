...
Wolf_Surface     *latchpics[NUMLATCHPICS];
...
boolean FizzleFade (Wolf_Surface *source, int x1, int y1,
    unsigned width, unsigned height, unsigned frames, boolean abortable)
{
    ...
    frame = GetTimeCount();
    byte *srcptr = source->pixels;
    do
    {
        if(abortable && IN_CheckAck ())
        {
            VL_ScreenToScreen(wolf_screen,source);
            VL_RefreshScreen();
            return true;
        }

        byte *destptr = wolf_screen->pixels;
        ...
        for(int i = first; i < 2; i++)
        {
            for(unsigned p = 0; p < pixperframe; p++)
            {
                ...
                if(screenBits == 8)
                {
                    *(destptr + (y1 + y) * wolf_screen->pitch + x1 + x)
                        = *(srcptr + (y1 + y) * source->pitch + x1 + x);
                }
                else
                {
                    byte col = *(srcptr + (y1 + y) * source->pitch + x1 + x);
                    destptr[(y1 + y) * wolf_screen->pitch + (x1 + x)] = col;
                }
