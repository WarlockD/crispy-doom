...
typedef enum Wolf_VideoFlags {
	WOLF_SURFACE_LOCK = 1,
} Wolf_VideoFlags;

typedef struct Wolf_Palette {
	union {
		uint32_t data[256];
		Wolf_Color colors[256];
	};
} Wolf_Palette;


typedef struct Wolf_Surface {
	int lock;
	uint8_t flags;
	int w;
	int h;
...
Wolf_Surface* VL_CreateSurface(int width, int height);
//void VL_BlitSurface(Wolf_Surface* dst, const Wolf_Rect* dst_rect, Wolf_Surface* src, const Wolf_Rect* src_rect);
...
extern  Wolf_Surface     *latchpics[NUMLATCHPICS];
...
#define GetTicks() ((GetTicksMS()*7)/100)
